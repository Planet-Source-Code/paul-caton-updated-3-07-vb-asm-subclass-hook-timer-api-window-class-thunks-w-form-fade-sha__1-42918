...
Implements WinSubHook.iSubclass                                       'Guarantee that we'll implement the iSubclass interface
Implements WinSubHook.iWindow                                         'Guarantee that we'll implement the iWindow interface
...
'Why a method for FadeOut rather than a property like FadeIn? - 'Cos on an unload of a form at app end will
'take the fader with it shortly after fade start, or done another way prevent the form unload, or...
'
...
'This UserControl is invisible at runtime therefore the paint event only fires at design time
Private Sub UserControl_Paint()
Const COL_APPWORKSPACE  As Long = 12, _
      COL_BTNFACE       As Long = 15, _
...
'we tend to want to do whatever preemptively before the parent.
End Sub
